...
```{r}
df <- read.csv('../data/data/processed/processed.csv', header=TRUE, skipNul=TRUE,
               na.strings=c("","NA"))
...
reference directory (`references/readable-column-names.csv`). Let's load and
apply these updated names to our data frame.

```{r}
new_col_names <- readr::read_lines('../references/readable-column-names.csv')
stopifnot(identical(length(new_col_names)+0, c(490)))
...
Drop rows for entries that did not complete the treatment response phase of the
study (i.e., phase 2).

```{r}
df <- df %>%
  filter(phase_2_complete == 2)
stopifnot(identical(dim(df)+0, c(2322, 490)))
dim(df)
```
...
stopifnot(identical(dim(df)+0, c(2322, 274)))
dim(df)
```
...
#df$valium_type <- as.factor(df$valium_type)
#df$other_drug_desc <- as.factor(df$other_drug_desc)

str(df, list.len=5)
```

# Drop Entries Without Fear/Anxiety
...
stopifnot(identical(dim(df)+0, c(1308, 275)))
dim(df)
```
...
```{r}
dir.create(path='../build/', showWarnings=F)
