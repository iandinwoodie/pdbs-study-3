...
str(df)
stopifnot(identical(dim(df)+0, c(1305, 280)))
```
...
```{r fa_overall_glm, warning=FALSE}
# Fit model to data.
...
```{r fa_inanimate_glm, warning=FALSE}
# Fit model to data.
f <- as.formula(paste0(outcome, "~", "."))
m <- glm(f, data=df_red, family="binomial")
summary(m)
...
```{r fa_animate_glm, warning=FALSE}
# Fit model to data.
f <- as.formula(paste0(outcome, "~", "."))
m <- glm(f, data=df_red, family="binomial")
summary(m)
...
```{r fa_situational_glm, warning=FALSE}
# Fit model to data.
f <- as.formula(paste0(outcome, "~", "."))
m <- glm(f, data=df_red, family="binomial")
summary(m)
...
```{r fa_generalized_glm, warning=FALSE}
# Fit model to data.
f <- as.formula(paste0(outcome, "~", "."))
m <- glm(f, data=df_red, family="binomial")
summary(m)
...
#df_red <- subset(df_red, select=-consultant_cred) # VIF 12.67

summary(df_red)
```

```{r fa_ptsd_glm, warning=FALSE}
# Fit model to data.
f <- as.formula(paste0(outcome, "~", "."))
m <- glm(f, data=df_red, family="binomial")
summary(m)
